OPENQASM 3.0;
// RUN: qe-compiler -X=qasm --emit=ast-pretty %s | FileCheck %s --check-prefix AST-PRETTY
// RUN: qe-compiler -X=qasm --emit=mlir %s --enable-circuits=false| FileCheck %s --check-prefixes MLIR,MLIR-NO-CIRCUITS
// RUN: qe-compiler -X=qasm --emit=mlir %s --enable-circuits | FileCheck %s  --check-prefixes MLIR,MLIR-CIRCUITS
